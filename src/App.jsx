...
const WA_PRETEXT = encodeURIComponent("Hi Makrees! 👋 I'm interested in your services and would like to know more. Can you help me?");
...
function WhatsAppWidget() {
  ...
  return (
    <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 12 }}>
      {open && (
        <div style={{ background: "#fff", borderRadius: 20, boxShadow: "0 8px 40px rgba(0,0,0,.18)", width: 300, overflow: "hidden", animation: "slideUp .3s ease" }}>
          {/* Header */}
          <div style={{ background: "linear-gradient(135deg,#25D366,#128C7E)", padding: "18px 16px", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: "50%", overflow: "hidden", border: "2px solid rgba(255,255,255,.4)", flexShrink: 0 }}>
              <img src="/makrees_logo.jpeg" alt="Makrees" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
            <div>
              <div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Makrees Support</div>
              <div style={{ color: "rgba(255,255,255,.85)", fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#90ee90", display: "inline-block" }} /> Online
              </div>
            </div>
            <button onClick={() => setOpen(false)} style={{ marginLeft: "auto", background: "none", border: "none", color: "rgba(255,255,255,.8)", cursor: "pointer", padding: 4 }}><IconClose /></button>
          </div>
          {/* Chat bubble */}
          <div style={{ padding: "20px 16px", background: "#f0f0f0" }}>
            <div style={{ background: "#fff", borderRadius: "0 12px 12px 12px", padding: "12px 14px", boxShadow: "0 1px 3px rgba(0,0,0,.1)", maxWidth: "85%", marginBottom: 16 }}>
              <p style={{ fontSize: 13, color: "#1a1a2e", lineHeight: 1.6, margin: 0 }}>
                👋 Hi there! Welcome to <strong>Makrees</strong>.<br /><br />How can we help you today?
              </p>
              <div style={{ fontSize: 11, color: "#9AA0B4", marginTop: 6 }}>Makrees Team</div>
            </div>
            {/* Quick reply options */}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <a key={opt} href={`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(opt)}`} target="_blank" rel="noopener noreferrer"
                  style={{ background: "#fff", border: "1.5px solid #25D36630", borderRadius: 24, padding: "9px 14px", fontSize: 12, color: "#1a1a2e", fontWeight: 600, textDecoration: "none", textAlign: "center", transition: "all .2s", display: "block" }}>
                  {opt}
                </a>
              ))}
            </div>
          ...
          {/* Live chat button */}
          <a href={`https://wa.me/${WA_NUMBER}?text=${WA_PRETEXT}`} target="_blank" rel="noopener noreferrer"
            style={{ display: "block", background: "linear-gradient(135deg,#25D366,#128C7E)", color: "#fff", textAlign: "center", padding: "14px", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
            💬 Start Live Chat on WhatsApp
          </a>
        </div>
      )}
      {/* Float button */}
      <button onClick={() => setOpen(!open)} style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,#25D366,#128C7E)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(37,211,102,.5)", transition: "transform .2s", color: "#fff" }}
